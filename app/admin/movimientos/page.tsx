...
export default function MovimientosPage() {
  const operationModules = MOVEMENT_MODULES.filter((item) => item.category === "Compras")
  ...
  return <div className="space-y-5">
    ...
    <Card><CardContent className="flex min-h-80 flex-col items-center justify-center p-8 text-center"><span className="rounded-2xl bg-primary/10 p-4 text-primary"><FileCheck2 className="h-8 w-8" /></span><h2 className="mt-4 text-xl font-bold">Selecciona un movimiento desde el menú lateral</h2><p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">Este centro conserva las operaciones de compras. Los documentos de venta y todas las operaciones POS ahora se gestionan desde el módulo de Ventas.</p><div className="mt-5 flex items-center gap-2 rounded-lg bg-muted px-4 py-2 text-xs text-muted-foreground"><Receipt className="h-4 w-4" />Cada opción conserva su ruta, alcance, efectos e integración regulatoria.</div></CardContent></Card>
  </div>
}
